...
function monthExtends(monthorday){
 ...
 function oneEventClick(date) {
	 var start = new Date(date);
	clickDate = start.getFullYear() + "" + monthExtends(start.getMonth() + 1) + "" + monthExtends(start.getDate());
    editWindow.show();
    var grid = mini.get("datagrid1");
    grid.load({clickDate: clickDate});
}
	...
	if($('.calendar').length > 0){
		$('.calendar').fullCalendar({
		    dayClick: function(date, allDay, jsEvent, view) {
		    	  clickDate = view.calendar.formatDate(date,'yyyyMMdd');
		          editWindow.show();
		          var grid = mini.get("datagrid1");
		          grid.load({clickDate: clickDate});
		    },   
			...
			events: function(start, end, callback) {
				$.ajax({
					type : "post",
					dataType : "json",
					url : pageContext_request_contextPath + "teachingManagement/getCourseArrangement.do?who=manager",
					data : {
						currentCalendarStartDate : start.getFullYear() + "" + monthExtends(start.getMonth() + 1) + "" + monthExtends(start.getDate()),
						currentCalendarEndDate : end.getFullYear() + "" + monthExtends(end.getMonth() + 1) + "" + monthExtends(end.getDate())
					},
						...
						//alert(text[0].start);
						  ...
						  for(var i = 0 ; i < text.length ; i++){
							  events.push({
			                        ...
			                        start:
			                        	new Date(
			                        		parseInt(String(text[i].start).split(",")[0]),
			                        		parseInt(String(text[i].start).split(",")[1]),
			                        		parseInt(String(text[i].start).split(",")[2]),
			                        		parseInt(String(text[i].start).split(",")[3]),
			                        		parseInt(String(text[i].start).split(",")[4])),
			                        end: new Date(
			                        		parseInt(String(text[i].end).split(",")[0]),
			                        		parseInt(String(text[i].end).split(",")[1]),
			                        		parseInt(String(text[i].end).split(",")[2]),
			                        		parseInt(String(text[i].end).split(",")[3]),
			                        		parseInt(String(text[i].end).split(",")[4])),
			                        allDay:false,
			                        url:String("javascript:oneEventClick('" +
			                        		new Date(
					                        		parseInt(String(text[i].start).split(",")[0]),
					                        		parseInt(String(text[i].start).split(",")[1]),
					                        		parseInt(String(text[i].start).split(",")[2])
